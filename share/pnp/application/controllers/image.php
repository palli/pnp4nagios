...
class Image_Controller extends System_Controller  {
    ...
    public function index()
    {
        ...
        if($this->input->get('w') != "" )
            $this->config->conf['graph_width'] = intval($this->input->get('w'));
        if($this->input->get('graph_width') != "" )
            $this->config->conf['graph_width'] = intval($this->input->get('graph_width'));

        if($this->input->get('h') != "" )
            $this->config->conf['graph_height'] = intval($this->input->get('h'));
        if($this->input->get('graph_height') != "" )
            $this->config->conf['graph_height'] = intval($this->input->get('graph_height'));
        ...
        if(isset($this->tpl)){
            ...
            $image = $this->backend->doImage($this->data->STRUCT[0]['RRD_CALL']);
            $this->backend->streamImage($image);
            ...
            if($this->auth->is_authorized($this->data->MACRO['AUTH_HOSTNAME'], $this->data->MACRO['AUTH_SERVICEDESC']) === FALSE)
                $this->backend->streamImage("ERROR: NOT_AUTHORIZED"); 
